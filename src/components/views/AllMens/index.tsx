...
export default function AllMens({ particularDatas, apiAllProductData, navigaitonData }: any) {
  ...
  const [allProductData, setallProductData] = useState(apiAllProductData.data1.data.collection.products.edges);
  const [particularProductData, setParticularProductData] = useState(particularDatas.productData);
      ...
      <div>
        ...
        <div className={`${inter.className} py-16 w-full bg-white px-5`}>
          <div className="gap-4 sm:gap-6 max-w-[77rem] justify-center sm:justify-start mx-auto flex flex-wrap">
            <div className="w-5/12 md:sm:w-2/12 lg:w-3/12 h-[32rem]">
              <ul className="space-y-4">
                <li onClick={() => { filterData("data1") }} className={`text-xl cursor-pointer ${active == 1 ? "font-semibold" : "font-medium"}`} >{navigaitonData[0]}</li>
                <li onClick={() => { filterData("data2"); }} className={`hover:ml-2 duration-500 cursor-pointer text-xl text-gray-800 ${active == 2 ? "font-semibold" : "font-medium"}`}>{navigaitonData[1]}</li>
                <li onClick={() => { filterData("data3"); }} className={`hover:ml-2 duration-500 cursor-pointer text-xl text-gray-800 ${active == 3 ? "font-semibold" : "font-medium"}`}>{navigaitonData[2]}</li>
                <li onClick={() => { filterData("data4"); }} className={`hover:ml-2 duration-500 cursor-pointer text-xl text-gray-800 ${active == 4 ? "font-semibold" : "font-medium"}`}>{navigaitonData[3]}</li>
                <li onClick={() => { filterData("data5"); }} className={`hover:ml-2 duration-500 cursor-pointer text-xl text-gray-800 ${active == 5 ? "font-semibold" : "font-medium"}`}>{navigaitonData[4]}</li>
                <li onClick={() => { filterData("data6"); }} className={`hover:ml-2 duration-500 cursor-pointer text-xl text-gray-800 ${active == 6 ? "font-semibold" : "font-medium"}`} tabIndex={0}>{navigaitonData[5]}</li>
              </ul>
            </div>
            {
