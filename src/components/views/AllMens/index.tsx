...
export default function AllMens({ particularDatas }: any) {
  ...
  function filterData(labelToMatch: string) {
    let updatedData = particularDatas.productData.filter(
      (item: any) => item.catogry2 == labelToMatch
    );
    setParticularProductData(updatedData);
  }
  function filterDataCatogry1(labelToMatch: string) {
    let updatedData = particularDatas.productData.filter(
      (item: any) => item.catogry1 == labelToMatch
    );
    setParticularProductData(updatedData);
  }
            ...
            <div className="w-5/12 md:sm:w-2/12 lg:w-3/12 h-[32rem]">
              <ul className="space-y-4">
                <li
                  onClick={() => {
                    filterDataCatogry1(
                      particularDatas.NavigationData.catogryToShow
                    );
                  }}
                  className="text-xl cursor-pointer focus:font-semibold"
                  tabIndex={0}
                >
                  {particularDatas.NavigationData.catogry}
                </li>
                {particularDatas.NavigationData.labels &&
                  particularDatas.NavigationData.labels.map(
                    (item: NavigationDataLabelType, index: number) => (
                      <li
                        key={index + 768}
                        onClick={() => {
                          filterData(item.catogry);
                        }}
                        className="hover:ml-2 duration-500 cursor-pointer text-xl text-gray-800 focus:font-semibold"
                        tabIndex={0}
                      >
                        {item.label}
                      </li>
