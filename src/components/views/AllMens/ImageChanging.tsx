...
export default function ImageChanging({ item, index }: any) {
    ...
    return (
        ...
        <div onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} key={index + 435} className="group relative flex flex-col space-y-2 w-5/12 sm:w-[23%]">
            {isHovering ? (
                <Image className="h-[24rem] object-cover" width={500} height={500} src={item.imageUrl2} alt={item.imageAlt} />
            ) : (
                <Image className="h-[24rem] object-cover" width={500} height={500} src={item.imageUrl} alt={item.imageAlt} />
            )}
